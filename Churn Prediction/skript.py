...
df_nn = df[['gender', 'SeniorCitizen', 'Partner', 'Dependents',
...
for i in range(df_nn.shape[1]):
    plt.subplot(6, 3, i + 1)
    f = plt.gca()
    f.set_title(df_nn.columns.values[i])

    vals = np.size(df_nn.iloc[:, i].unique())
    if vals >= 100:
        vals = 100
    
    plt.hist(df_nn.iloc[:, i], bins=vals, color = '#ec838a')
...
df_nn = df[['SeniorCitizen', 'Partner', 'Dependents',
       'tenure', 'PhoneService', 'PaperlessBilling',
        'MonthlyCharges', 'TotalCharges']]

correlations = df_nn.corrwith(df.Churn)
...
df_nn[['MonthlyCharges', 'TotalCharges']].plot.scatter(figsize = (15, 10), x = 'MonthlyCharges',
...
df_nn = df_nn.drop(columns = "TotalCharges")
...
#Applying changes in the main df:
...
#Rejoin userid to df (column concatenation)

df = pd.concat([df, identity], axis = 1)

#Step 11: Split df into dependent and independent variables-----------------------------------
...
# Fit Logistic Regression on the Training df:
